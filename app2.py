...
st.markdown(
    "ローカルPython / Streamlit UI版（組織ピックアップ・ハイブリッド対応）"
)
      ...
      if missing_cols:
        ...
        with col1:
          ...
          selected_org_large = st.selectbox(
              "組織-大で絞り込み", org_large_list, key="t1_org_l"
          )
        ...
        st.markdown(
            "絞り込まれたリストから、今回対象とするメンバーにチェックを入れてください。"
        )
        ...
        if st.button("📥 組織リスト（Excel）を作成"):
          if len(selected_rows) == 0:
            ...
            # 組織名や参加費をデータフレームに追加（9フィールド目等の出力用）
            ...
            with pd.ExcelWriter(output, engine="openpyxl") as writer:
              selected_rows.to_excel(
                  writer, index=False, sheet_name="組織リスト"
              )
